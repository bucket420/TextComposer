...
void Chord::setWaveTable(std::array<double, 25> scale, std::string scaleType)
{
	...
	this->Wave::setWaveTable(scale[step], duration);

	if (chord[romanNumber.size()] == 'd' || (chord.size() > romanNumber.size() + 1 && chord[romanNumber.size() + 1] == 'd'))
	{
		this->add(std::unique_ptr<Wave>(new Wave(scale[step + 3], duration)));
		this->add(std::unique_ptr<Wave>(new Wave(scale[step + 6], duration)));
	}
	else if (chord[romanNumber.size()] == 'a' || (chord.size() > romanNumber.size() + 1 && chord[romanNumber.size() + 1] == 'a'))
	{
		this->add(std::unique_ptr<Wave>(new Wave(scale[step + 4], duration)));
		this->add(std::unique_ptr<Wave>(new Wave(scale[step + 8], duration)));
	}
	else if (isupper(chord[0]))
	{
		this->add(std::unique_ptr<Wave>(new Wave(scale[step + 4], duration)));
		this->add(std::unique_ptr<Wave>(new Wave(scale[step + 7], duration)));
	}
